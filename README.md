...
- **3つのUIモード**: GUIデスクトップアプリ、コンソールモード、Webブラウザモード

### 高度な機能
- **タイミング調整**: システムオーディオとマイク入力の同期オフセット設定（±1000ms）
- **キー変更**: 原曲のピッチを半音単位で変更（±12半音）カラオケのキーコン機能
- **セッション録音**: ピッチデータとオーディオを自動録音・保存
- **データエクスポート**: CSV/JSON形式でデータをエクスポート
- **音量メーター**: マイクとシステムオーディオのレベルを視覚的に表示
- **一時停止/再開**: 分析を一時的に停止・再開可能

### UI機能
- **GUIベース設定**: デバイス選択、表示オプション、分析パラメータを直感的に設定
- **複数の表示モード**: シンプル、詳細、プロフェッショナルの3つの表示バリエーション
- **設定の永続化**: 設定を自動保存し、次回起動時に復元
...
デスクトップアプリケーションが起動します。メニューから設定を開いてデバイスを選択し、「開始」ボタンをクリックするだけで使用できます。

**特徴:**
- 直感的なGUIで設定可能
- デバイス選択が簡単
- 3つの表示モード（シンプル/詳細/プロフェッショナル）
- 設定の保存・読み込み
...
1. **起動**: `python gui_app.py` を実行
2. **設定**: メニューバー → 「ファイル」→「設定」
3. **デバイス選択**: オーディオタブでマイクとシステムオーディオデバイスを選択
4. **開始**: 「開始」ボタンをクリック
5. **分析**: マイクに向かって歌う
...
**高度な設定** ⭐NEW!
- **タイミング調整**: システムオーディオとマイク入力の同期オフセット（±1000ms）
  - 正の値: システムオーディオを遅らせる
  - 負の値: マイク入力を遅らせる
- **キー変更**: 原曲のピッチを半音単位で変更（±12半音）
  - カラオケのキーコン機能と同等
  - 歌いやすいキーに調整可能
- **録音設定**:
  - セッション録音の有効化/無効化
...
├── gui_app.py               # GUIモードのエントリーポイント（NEW!）
...
│   ├── config.py            # 設定管理
│   ├── session.py           # セッション録音・管理（NEW!）
│   ├── audio_utils.py       # 音量メーター（NEW!）
│   └── ui.py               # UI/可視化
├── examples/
│   └── simple_pitch_test.py # シンプルなサンプル
└── tests/
    └── test_pitch_detection.py  # 単体テスト
```
